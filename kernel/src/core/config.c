...
void fconfig_init(const char* data, size_t length) {
	...
	while (length > 0 && line) {
		const char* eol = simple_strnpbrk(line, "\r\n", length);
		...
		const char* equal_sign = simple_strnchr(line, '=', line_length);

		if (equal_sign) {
			key = line;

			// skip leading whitespace in the key
			while (simple_isspace(*key)) {
				++key;
			}
			key_length = equal_sign - key;

			// skip trailing whitespace in the key
			while (key_length > 0 && simple_isspace(key[key_length - 1])) {
				--key_length;
			}
			...
			if (!eol) {
				value_length = simple_strnlen(value, value_length);
			}

			// skip trailing whitespace in the value
			while (value_length > 0 && simple_isspace(value[value_length - 1])) {
				--value_length;
			}
...
ferr_t fconfig_get(const char* key, char** out_value) {
	...
	simple_memcpy(new, orig, orig_len);
	new[orig_len] = '\0';
...
const char* fconfig_get_nocopy(const char* key, size_t* out_value_length) {
	for (size_t i = 0; i < config_entry_count; ++i) {
		...
		if (simple_strncmp(entry->key, key, entry->key_length) == 0) {
			if (out_value_length) {
