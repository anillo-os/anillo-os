...
#if VFSMAN_BOOTSTRAP
	#include <vfs_handoff.server.h>
#else
	#include <vfs_handoff.client.h>
#endif
...
#if VFSMAN_BOOTSTRAP
static sys_event_t channel_destroyed_event = SYS_EVENT_INIT;
...
static void channel_dtor(void* context) {
	sys_event_notify(&channel_destroyed_event);
};

ferr_t vfsman_handoff_handoff_impl(void* _context, sys_server_channel_t** server, sys_data_t** out_ramdisk_data, ferr_t* out_status) {
	ferr_t status = ferr_ok;
	eve_server_channel_t* eve_server_channel = NULL;
	sys_server_channel_t* sys_server_channel = NULL;
	sys_data_t* ramdisk_data = NULL;

	status = sys_data_create_from_shared_memory((void*)&ramdisk_memory, 0, ramdisk_memory_page_count, &ramdisk_data);
	if (status != ferr_ok) {
		goto out;
	}

	status = vfsman_detach(eve_loop_get_current(), &eve_server_channel);
	if (status != ferr_ok) {
		goto out;
	}

	status = eve_server_channel_target(eve_server_channel, true, &sys_server_channel);
	if (status != ferr_ok) {
		goto out;
	}

	eve_item_set_destructor(eve_server_channel, channel_dtor);

	eve_release(eve_server_channel);
	eve_server_channel = NULL;

	// wait until we've confirmed that no one else is holding onto the server channel
	eve_event_wait(&channel_destroyed_event);

	*server = sys_server_channel;
	sys_server_channel = NULL;

	*out_ramdisk_data = ramdisk_data;
	ramdisk_data = NULL;
...
out:
	...
	if (eve_server_channel) {
		eve_release(eve_server_channel);
	}
	if (ramdisk_data) {
		sys_release(ramdisk_data);
	}
	*out_status = status;
	return ferr_ok;
};
...
void start(void) {
	...
	sys_abort_status_log(vfsman_handoff_serve(main_loop, NULL));
...
void main(void) {
	...
	ferr_t call_status = ferr_ok;
	ferr_t remote_status = ferr_ok;
	sys_shared_memory_t* ramdisk_memory = NULL;
	sys_data_t* ramdisk_data = NULL;

	main_loop = eve_loop_get_main();

	sys_abort_status_log(vfsman_handoff_handoff(NULL, &server_channel, &ramdisk_data, &remote_status));
	sys_abort_status_log(remote_status);

	sys_abort_status_log(sys_data_shared_memory(ramdisk_data, &ramdisk_memory));
	sys_release(ramdisk_data);

	vfsman_init();
	vfsman_ramdisk_init(ramdisk_memory);

	sys_abort_status_log(vfsman_serve_explicit(eve_loop_get_main(), server_channel));
