 ...
 *
 * @param[out] pixel Pointer to a pixel structure to copy the values of the framebuffer pixel into. If this is `NULL`, the function is a no-op.
 ...
 *
 * Return values:
 * @retval ferr_ok                Successfully retrieved pixel information into @p pixel.
 * @retval ferr_invalid_argument  Either @p x or @p y was out-of-bounds for the framebuffer. @p pixel remains unmodified and the framebuffer is never accessed.
 * @retval ferr_temporary_outage  No framebuffer is available. @p pixel remains unmodified.
 */
 ...
 * 
 * @param[in] pixel Read-only pointer to a pixel structure for the new values of the pixel. If this is `NULL`, the function is a no-op.
 ...
 *
 * Return values:
 * @retval ferr_ok                Successfully assigned pixel information from @p pixel into the framebuffer.
 * @retval ferr_invalid_argument  Either @p x or @p y was out-of-bounds for the framebuffer. The framebuffer remains unmodified and @p pixel is never accessed.
 * @retval ferr_temporary_outage  No framebuffer is available. @p pixel is never accessed.
 */
 ...
 *
 * @param[in] pixel Read-only pointer to a single pixel structure whose values will overwrite the values of all the pixel in the given area of the framebuffer.
 * @param[in]  area Pointer to a structure describing the area to be overwritten.
 *
 * Return values:
 * @retval ferr_ok                Successfully assigned pixel information from @p pixel into the framebuffer.
 * @retval ferr_invalid_argument  The area was out-of-bounds for the framebuffer. The framebuffer remains unmodified and @p pixel is never accessed.
 * @retval ferr_temporary_outage  No framebuffer is available. @p pixel and @p area are never accessed.
 */
FERRO_WUR ferr_t ferro_fb_set_area_clone(const ferro_fb_pixel_t* pixel, const ferro_fb_rect_t* area);

/**
 * Copies the area described by @p old_area to the location described by @p new_area. The areas MUST be simple translations of each other.
 *
 * @param[in] old_area Pointer to a structure describing the area to copy pixels from.
 * @param[in] new_area Pointer to a structure describing the area to copy pixels to.
 *
 * Return values:
 * @retval ferr_ok                Successfully copied the pixels from the area described by @p old_area to the area described by @p new_area.
 ...
 * @retval ferr_temporary_outage  No framebuffer is available. @p old_area and @p new_area are never accessed.
 */
